...
const { toCursorAgentRelativePath } = require('./cursor-agent-names.cjs');
const { LEGACY_INSTALL_TARGETS, parseInstallArgs } = require('./install/request.cjs');
const {
  SUPPORTED_INSTALL_TARGETS,
  listLegacyCompatibilityLanguages,
  resolveLegacyCompatibilitySelection,
  resolveInstallPlan,
} = require('./install-manifests.cjs');
const { getInstallTargetAdapter } = require('./install-targets/registry.cjs');
...
function createStatePreview(options) {
  const { createInstallState } = require('./install-state.cjs');
  return createInstallState(options);
}

function applyInstallPlan(plan) {
  const { applyInstallPlan: applyPlan } = require('./install/apply.cjs');
